...
It comes in two variants that differ only in how a blob is keyed: the hash-keyed one below, written on request as `normalized-hash`, and the [dedup-id](#dedup-id-normalized-schema) one that the tools write by default.

The hash-keyed variant stores all tile blobs in the `images` table, and stores the tile Z,X,Y coordinates in a `map` table.
...
Optionally, hash-keyed normalized `.mbtiles` files can include a `tiles_with_hash` view.
Every `normalized-hash` file the `mbtiles` tool creates contains this view.

```sql
--8<-- "files/init-normalized-with-hash.sql"
```

### Dedup-id normalized schema
...
This is what `normalized` means to `mbtiles copy --mbtiles-type` and `martin cp --mbtiles-type`, so both write `tiles_shallow`/`tiles_data`.
Pass `normalized-hash` instead to get the `map` + `images` schema above.

The `tile_data_id` values are handed out as tiles are written, so they say nothing about a tile's content and two files holding the same tiles may number them differently.
In particular, `mbtiles apply-patch` reuses the numbering of the file it patches, which is not the numbering a fresh copy of the patched result would produce.
