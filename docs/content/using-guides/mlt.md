...
The `auto` preset is guaranteed to work on a resonably new maplibre version, so for example `tessellation` will only be enabled if this has larger.
...
In most cases, `mlt: auto` is the right choice. Only tune these settings if you've profiled your tiles and identified a specific bottleneck.

## Combining with Compression

MLT conversion and compression can be combined.

!!! warning

    MLT uses lightweight compression.
    This means that if you layer heavyweight compression on top of this, you remove most of the reasons for using it.
