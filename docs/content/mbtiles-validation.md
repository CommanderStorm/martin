...
For the [dedup-id normalized](mbtiles-schema.md#dedup-id-normalized-schema) schema variant, foreign key integrity is validated instead - every `tile_data_id` in `tiles_shallow` must have a matching entry in `tiles_data`.
